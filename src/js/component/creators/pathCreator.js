...
export default class PathCreator extends Phaser.GameObjects.Container {
  /**
   * Creates PathCreator
   * @param {TowerDefenseScene} scene Scene object where this will draw
   */
  constructor (scene) {
    super(scene);
    this.enabled = false;
    ...
    this.points = [];
    this.clicked = false;
    this.pathPointerTextureKey = 'pathPointer';
    this.createPathPointer();
    this.scene.add.existing(this);
  }

  createPathPointer () {
    if (!this.scene.textures.exists(this.pathPointerTextureKey)) {
      const graphics = this.scene.add.graphics();
      graphics.fillStyle(color.SECONDARY.NUMBER);
      graphics.fillCircle(32, 32, 32);
      graphics.generateTexture(this.pathPointerTextureKey, 64, 64);
      graphics.destroy();
    }
  }
  ...
  enable () {
    ...
    if (this.enableCallback && this.enableContext) {
      this.enableCallback.apply(this.enableContext);
    }
  ...
  disable () {
    ...
    if (this.disableCallback && this.disableContext) {
      this.disableCallback.apply(this.disableContext);
    }
  ...
  init () {
    ...
    this.on(Phaser.Input.Events.POINTER_MOVE, pointer => {
      ...
      if (pointer.isDown && this.enabled) {
        ...
        const pointImage = this.scene.add.image(
          pointer.x,
          pointer.y,
          this.pathPointerTextureKey
        );
